...
import com.firefox.center.sys.core.system.mapper.SysCategoryMapper;
...
public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory> implements ISysCategoryService {
	...
	@Override
	public List<TreeSelectModel> queryListByCode(String pcode) throws FirefoxException {
		...
		if(oConvertUtils.isNotEmpty(pcode)) {
			...
			if(list==null || list.size() ==0) {
				throw new FirefoxException("该编码【"+pcode+"】不存在，请核实!");
			}
			if(list.size()>1) {
				throw new FirefoxException("该编码【"+pcode+"】存在多个，请核实!");
			}
