 ...
 * 封装 access_token
 */
public class PayAccessToken implements RetryUtils.ResultCheck, Serializable {
    ...
    public PayAccessToken(String jsonStr) {
        this.json = jsonStr;
        try {
            ...
            if(temp.getIntValue("code")==0){
                ...
                expires_in = 7200;
                errcode = null;
                errmsg = temp.getString("message");
                expiredTime = System.currentTimeMillis() + ((expires_in - 9) * 1000);
            }else{
                errcode = temp.getIntValue("code");
                errmsg = temp.getString("message");
            }
        } catch (Exception e) {
