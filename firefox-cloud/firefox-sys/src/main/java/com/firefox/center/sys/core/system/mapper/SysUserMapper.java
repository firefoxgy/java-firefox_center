...
public interface SysUserMapper extends BaseMapper<SysUser> {
	...
	@Select("select su.* from base_corp_admin bca left join sys_user su on bca.user_id=su.id ${ew.customSqlSegment}")
	List<SysUser> selectCorpAdminUserPage(IPage<SysUser> page, @Param(Constants.WRAPPER) Wrapper wrapper);

	@Select("<script>"+
			"select * from sys_user " +
			"where 1=1 " +
			"<if test='cId != null'> " +
			"and id not in(select user_id from base_corp_admin where c_id = #{cId}) " +
			"</if> " +
			...
			"order by username"+
			"</script>")
	List<SysUser> selectCorpAdminAddUserPage(IPage<SysUser> page, Integer cId, SysUser user);
}
