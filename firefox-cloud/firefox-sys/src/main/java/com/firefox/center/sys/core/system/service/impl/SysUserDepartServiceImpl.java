...
import com.firefox.center.sys.core.system.entity.SysUserDepart;
import com.firefox.center.sys.core.system.mapper.SysUserDepartMapper;
...
import com.firefox.center.sys.core.system.service.ISysUserDepartService;
...
public class SysUserDepartServiceImpl extends ServiceImpl<SysUserDepartMapper, SysUserDepart> implements ISysUserDepartService {
	...
	public List<SysUser> queryUserByDepCode(String depCode,String realname) {
		...
		if(uDepList != null && uDepList.size() > 0) {
			for(SysUserDepart uDep : uDepList) {
				if(!Consts.User.SUPER_ID.equals(uDep.getUserId())){
					userIdList.add(uDep.getUserId());
				}
			}
