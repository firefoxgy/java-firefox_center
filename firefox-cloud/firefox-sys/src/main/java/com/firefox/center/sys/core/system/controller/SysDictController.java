...
import com.firefox.center.sys.common.system.vo.DictQuery;
import com.firefox.center.sys.common.system.vo.LoginUser;
import com.firefox.center.sys.common.util.ImportExcelUtil;
import com.firefox.center.sys.common.util.SqlInjectionUtil;
import com.firefox.center.sys.common.util.oConvertUtils;
import com.firefox.center.sys.core.system.entity.SysDict;
import com.firefox.center.sys.core.system.entity.SysDictItem;
import com.firefox.center.sys.core.system.model.SysDictTree;
...
import com.firefox.center.sys.core.system.vo.SysDictPage;
...
public class SysDictController {
	...
	public R<IPage<SysDict>> queryPageList(SysDict sysDict,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
									  @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,HttpServletRequest req) {
		R<IPage<SysDict>> result = new R<IPage<SysDict>>();
	...
	public R<List<SysDictTree>> treeList(SysDict sysDict,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
									  @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,HttpServletRequest req) {
		R<List<SysDictTree>> result = new R<>();
	...
	public R<List<DictModel>> getDictItems(@PathVariable String dictCode, @RequestParam(value = "sign",required = false) String sign,HttpServletRequest request) {
	...
	public R<?> queryAllDictItems(HttpServletRequest request) {
	...
	public R<List<DictModel>> loadDict(@PathVariable String dictCode,@RequestParam(name="keyword") String keyword, @RequestParam(value = "sign",required = false) String sign,HttpServletRequest request) {
	...
	public R<List<String>> loadDictItem(@PathVariable String dictCode,@RequestParam(name="key") String keys, @RequestParam(value = "sign",required = false) String sign,HttpServletRequest request) {
	...
	public R<List<DictModel>> queryTableData(DictQuery query,
												  @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
												  @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
												  @RequestParam(value = "sign",required = false) String sign,HttpServletRequest request){
		R<List<DictModel>> res = new R<List<DictModel>>();
		// SQL注入漏洞 sign签名校验
	...
	public R<SysDict> add(@RequestBody SysDict sysDict) {
		R<SysDict> result = new R<SysDict>();
		try {
	...
	public R<SysDict> edit(@RequestBody SysDict sysDict) {
		R<SysDict> result = new R<SysDict>();
	...
	@CacheEvict(value=CacheConstant.Sys.SYS_DICT_CACHE, allEntries=true)
	public R<SysDict> delete(@RequestParam(name="id",required=true) String id) {
		R<SysDict> result = new R<SysDict>();
	...
	@CacheEvict(value= CacheConstant.Sys.SYS_DICT_CACHE, allEntries=true)
	public R<SysDict> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		R<SysDict> result = new R<SysDict>();
		if(oConvertUtils.isEmpty(ids)) {
			result.setSuccess(false);
			result.errorMsg("参数不识别！");
		}else {
			sysDictService.removeByIds(Arrays.asList(ids.split(",")).stream().filter(s->s.indexOf("_")==-1).collect(Collectors.toList()));
			result.okMsg("删除成功!");
		}
		return result;
	}
	...
	public R<?> refleshCache() {
		R<?> result = new R<SysDict>();
		//清空字典缓存
		Set keys = redisTemplate.keys(CacheConstant.Sys.SYS_DICT_CACHE + "*");
		Set keys2 = redisTemplate.keys(CacheConstant.Sys.SYS_DICT_TABLE_CACHE + "*");
	...
	public R<List<SysDict>> deleteList() {
		R<List<SysDict>> result = new R<List<SysDict>>();
	...
	public R<?> deletePhysic(@PathVariable String id) {
		try {
