...
import com.firefox.center.sys.core.system.entity.SysPosition;
import com.firefox.center.sys.core.system.service.ISysPositionService;
...
public class SysPositionController {
    ...
    public R<IPage<SysPosition>> queryPageList(SysPosition sysPosition,
                                                    @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                                    @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
                                                    HttpServletRequest req) {
        R<IPage<SysPosition>> result = new R<IPage<SysPosition>>();
    ...
    public R<SysPosition> add(@RequestBody SysPosition sysPosition) {
        R<SysPosition> result = new R<SysPosition>();
        try {
    ...
    public R<SysPosition> edit(@RequestBody SysPosition sysPosition) {
        R<SysPosition> result = new R<SysPosition>();
    ...
    public R<SysPosition> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
        R<SysPosition> result = new R<SysPosition>();
        if (ids == null || "".equals(ids.trim())) {
            result.setSuccess(false);
            result.errorMsg("参数不识别！");
        } else {
            this.sysPositionService.removeByIds(Arrays.asList(ids.split(",")).stream().filter(s->s.indexOf("_")==-1).collect(Collectors.toList()));
            result.okMsg("删除成功!");
        }
        return result;
    }
    ...
    public R<SysPosition> queryById(@RequestParam(name = "id", required = true) String id) {
        R<SysPosition> result = new R<SysPosition>();
    ...
    public ModelAndView exportXls(HttpServletRequest request, HttpServletResponse response) {
        ...
        mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("职务表列表数据", "导出人:Firefox", "导出信息"));
