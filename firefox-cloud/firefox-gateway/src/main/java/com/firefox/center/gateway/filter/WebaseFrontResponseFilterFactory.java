...
import com.firefox.center.common.constants.AddCheck;
...
@Slf4j
@Component
public class WebaseFrontResponseFilterFactory extends ModifyResponseBodyGatewayFilterFactory {
        ...
        return (exchange, resp) -> {
            ...
            if (response.getStatusCode() == HttpStatus.OK ) {
                ...
                try {
                    ...
                    JSONObject func = exchange.getAttribute(WebaseFrontFilterFactory.ATTRIBUTE_FUNC);
                    ...
                    final Function function = new Function(func.getString(WebaseFrontFilterFactory.JSON_NAME),
                        Arrays.<Type>asList(),
                        outs);
                    List<Type> results = FunctionReturnDecoder.decode(output, function.getOutputParameters());
                   respJson.put("ret",results);


                    String data = "{\"code\":" + code + ",\"message\":\"" + respJson.getString("message") + "\",\"data\": " + respJson.toJSONString() + "}";
    ...
    @Override
    public String name() {
        return "WebaseFrontResponseFilter";
    }
