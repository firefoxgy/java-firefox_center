        ...
        <a-tabs defaultActiveKey="1" >
          <a-tab-pane tab="客户信息" key="1">
            <div>
              ...
              <a-row type="flex" style="margin-bottom:10px" :gutter="16" v-for="(item, index) in orderMainModel.firefoxOrderCustomerList" :key="index">
                <a-col :span="5">
                  <a-form-item>
                    <a-input placeholder="客户名" v-decorator="['firefoxOrderCustomerList['+index+'].name', {'initialValue':item.name,rules: [{ required: true, message: '请输入用户名!' }]}]" />
                  </a-form-item>
                </a-col>
                <a-col :span="5">
                  <a-form-item>
                    <a-select placeholder="性别" v-decorator="['firefoxOrderCustomerList['+index+'].sex', {'initialValue':item.sex}]">
                ...
                <a-col :span="5">
                  <a-form-item>
                    <a-input placeholder="身份证号" v-decorator="['firefoxOrderCustomerList['+index+'].idcard', {'initialValue':item.idcard,rules: [{ pattern: '^\\d{6}(18|19|20)?\\d{2}(0[1-9]|1[012])(0[1-9]|[12]\\d|3[01])\\d{3}(\\d|[xX])$', message: '身份证号格式不对!' }]}]"/>
                  </a-form-item>
                </a-col>
                <a-col :span="5">
                  <a-form-item>
                    <a-input placeholder="手机号" v-decorator="['firefoxOrderCustomerList['+index+'].telphone', {'initialValue':item.telphone,rules: [{ pattern: '^1(3|4|5|7|8)\\d{9}$', message: '手机号格式不对!' }]}]"/>
                  </a-form-item>
                </a-col>
                <a-col :span="4">
                  <a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        title:"操作",
        visible: false,
        orderMainModel: {firefoxOrderCustomerList: [{}],
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.orderMainModel.firefoxOrderCustomerList = [{}];
        ...
        if(this.orderMainModel.id){
          ...
          getAction(this.url.orderCustomerList,params).then((res)=>{
            if(res.success){
              this.orderMainModel.firefoxOrderCustomerList = res.data;
              this.$forceUpdate()
            }
          })
          //初始化订单客户列表
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            let formData = {
              ...orderMainData,
              firefoxOrderCustomerList: orderMainData.firefoxOrderCustomerList,
      ...
      addRowCustom () {
        this.orderMainModel.firefoxOrderCustomerList.push({});
        this.$forceUpdate();
      },
      delRowCustom (index) {
        console.log(index)
        let all = this.form.getFieldsValue()
        all['firefoxOrderCustomerList'].splice(index,1);
        this.form.setFieldsValue(all)
        this.orderMainModel.firefoxOrderCustomerList.splice(index,1);
        this.$forceUpdate();
      },
      addRowTicket () {
